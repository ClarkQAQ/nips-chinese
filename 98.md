NIP-98
======

HTTP 认证
---------

`草稿` `可选`

本 NIP 定义了一个用于使用 nostr 事件授权对 HTTP 服务器请求的瞬态事件。

这对于为 Nostr 构建并处理 Nostr 用户账户的 HTTP 服务非常有用。

## Nostr 事件

使用一个 `kind 27235` 事件（参考 [RFC 7235](https://www.rfc-editor.org/rfc/rfc7235)）。

`content` 应为空。

必须包含以下标签：

* `u` - 绝对 URL
* `method` - HTTP 请求方法

示例事件：
```json
{
  "id": "fe964e758903360f28d8424d092da8494ed207cba823110be3a57dfe4b578734",
  "pubkey": "63fe6318dc58583cfe16810f86dd09e18bfd76aabc24a0081ce2856f330504ed",
  "content": "",
  "kind": 27235,
  "created_at": 1682327852,
  "tags": [
    ["u", "https://api.snort.social/api/v1/n5sp/list"],
    ["method", "GET"]
  ],
  "sig": "5ed9d8ec958bc854f997bdc24ac337d005af372324747efe4a00e24f4c30437ff4dd8308684bed467d9d6be3e5a517bb43b1732cc7d33949a3aaf86705c22184"
}
```

服务器必须执行以下检查来验证事件：
1. `kind` 必须是 `27235`。
2. `created_at` 时间戳必须在合理的时间窗口内（建议 60 秒）。
3. `u` 标签必须与绝对请求 URL（包括查询参数）完全相同。
4. `method` 标签必须与用于请求资源的 HTTP 方法相同。

当请求包含主体时（如 POST/PUT/PATCH 方法），客户端应在 `payload` 标签中包含请求体的 SHA256 哈希（十六进制），即 `["payload", "<sha256-hex>"]`，服务器可以检查此哈希以验证请求的载荷是否已获授权。

如果其中一项检查失败，服务器应以 401 Unauthorized 响应码进行响应。

服务器可以执行额外的实现特定验证检查。

## 请求流程

使用 `Authorization` HTTP 头，`kind 27235` 事件必须经过 base64 编码，并使用 `Nostr` 授权方案。

示例 HTTP Authorization 头：
```
Authorization: Nostr 

```

## 参考实现
- C# ASP.NET `AuthenticationHandler` [NostrAuth.cs](https://gist.github.com/v0l/74346ae530896115bfe2504c8cd018d3)
