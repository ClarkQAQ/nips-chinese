...
列表中的公共项目在事件 `tags` 数组中指定，而私有项目在 JSON 数组中指定，该数组模仿事件 `tags` 数组的结构，但被字符串化并使用与 [NIP-44](44.md) 相同的方案加密（共享密钥使用作者的公钥和私钥计算）并存储在 `.content` 中。此规范的早期版本使用 [NIP-04](04.md) 进行加密。这些现在已弃用。为了向后兼容，客户端可以通过在密文中搜索"iv"并相应地解密来自动发现加密是 NIP-04 还是 NIP-44。
...
| 表情符号            | 10030 | 用户首选表情符号和表情符号集指针            | `"emoji"`（参见 [NIP-30](30.md)）和 `"a"`（kind:30030 表情符号集）                                    |
| DM 中继器         | 10050 | 接收 [NIP-17](17.md) 直接消息的地方            | `"relay"`（参见 [NIP-17](17.md)）                                                                     |
| 优秀维基作者 | 10101 | [NIP-54](54.md) 用户推荐的维基作者               | `"p"`（公钥）                                                                                     |
| 优秀维基中继器  | 10102 | [NIP-54](54.md) 被认为只托管有用文章的中继器  | `"relay"`（中继器 URL）                                                                              |
...
| 表情符号集            | 30030 | 分类表情符号组                                                                     | `"emoji"`（参见 [NIP-30](30.md)）                                                     |
| 发布工件集 | 30063 | 软件发布的工件组                                                     | `"e"`（kind:1063 [文件元数据](94.md) 事件）、`"a"`（软件应用程序事件） |
