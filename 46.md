...
`content` 字段是类似 JSON-RPC 的消息，使用 [NIP-44](44.md) 加密，具有以下结构：
...
_远程签名器_ 可以通过使用 [NIP-05](05.md) 和 [NIP-89](89.md) 发布其元数据。使用 NIP-05，对 `<remote-signer>/.well-known/nostr.json?name=_` 的请求可能返回：
```jsonc
{
    "names":{
